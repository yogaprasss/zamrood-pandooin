...
const ExploreMoreButton = () => {
  return (
    <Link href='/destination' className='group flex items-center gap-3 cursor-pointer'>
      <svg width='40' height='40' viewBox='0 0 40 40' fill='none' xmlns='http://www.w3.org/2000/svg' className='stroke-[#004040] sm:group-hover:stroke-[#d1b775] transition-colors ease-in-out duration-300'>
      ...
      <span className='text-[#004040] sm:group-hover:text-[#d1b775] transition-colors ease-in-out duration-300 font-bold'>
        EXPLORE MORE
      </span>
    </Link>
  );
    ...
    <div className={`flex flex-col ${!isSmallCard && 'sm:flex-row'} gap-4`}>
      ...
      <div className='w-full flex flex-col justify-between'>
        <div>
          ...
          <h2 className='text-[#0b7373] text-xl sm:text-3xl font-bold line-clamp-2'>{title}</h2>
          <p className='text-[#004040] text-xs sm:text-sm font-bold my-2 hidden sm:block'>
            Organized by {organizer}
          </p>
          <p className='text-[#004040] text-sm sm:text-base line-clamp-4 my-2 sm:my-0'>{description}</p>
          <p className='text-[#004040] text-xs sm:text-sm font-bold block sm:hidden'>
            Organized by {organizer}
          </p>
        </div>
        <div className='flex justify-between items-end mt-8 sm:mt-0'>
          <div>
            ...
            <h3 className='text-[#0B7373] text-xl sm:text-2xl font-bold'>
              {formatCurrency(discountPrice || price)}
            </h3>
          </div>
          <Link
            href={`/destination/${slug}`}
            className='px-2.5 py-2 sm:px-4 sm:py-3 text-sm sm:text-base font-bold border-2 border-[#0b7373] text-[#0b7373] rounded-full sm:hover:bg-[#0b7373] sm:hover:text-[#faf9f5] duration-200'
          >
            See Details
          </Link>
        </div>
    ...
    <div id='destination' className='flex justify-center px-4 py-8 sm:py-12 w-full relative'>
      <div className='w-full max-w-7xl h-full'>
        <div className='flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6'>
          <h3 className='header-unbounded text-2xl sm:text-3xl text-[#004040] font-bold'>Destinations</h3>
          <ExploreMoreButton />
        </div>
        {isLoading && (
          <>
